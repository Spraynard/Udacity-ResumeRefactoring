...
view = {
	...
	renderSite : function () {
		...
		view.renderWork();
		view.renderProjects();
		view.renderEducation.Schools();
		view.renderEducation.Online();
	},

	renderWork : function () {

		//Renders all of the places that I have worked in the workExperience portion of the resumeIndex
		var workObject = controller.getWorkObj();

		for (var i = 0; i < workObject.length; i++) {
			$("#workExperience").append(HTMLbluestyling)
			$(".holder:last").append(HTMLworkStart);
			var employer = HTMLworkEmployer.replace(/%link%|%data%/gi, function(matched) {
				if (matched === "%link%") {
					return workObject[i].link;
				}
				else {
					return workObject[i].employer;
				}
			})

			var title = HTMLworkTitle.replace("%data%", workObject[i].title);
			var employerTitle = employer + title;
			var dates = HTMLworkDates.replace("%data%", workObject[i].dates);
			var description = HTMLworkDescription.replace("%data%", workObject[i].description);
	...
	renderProjects : function () {
		var projectObj = controller.getProjectsObj();

		for (var i = 0; i < projectObj.length; i++ ) {
			$("#projects").append(HTMLgreenstyling);
			$(".holder:last").append(HTMLprojectStart);

			var title = HTMLprojectTitle.replace("%data%",projectObj[i].title);
			var dates = HTMLprojectDates.replace("%data%",projectObj[i].dates);
			var description = HTMLprojectDescription.replace("%data%",projectObj[i].description);
			...
			$(".project-entry:last").append(lineBreak);

			var images = projectObj[i].images

			if (images.length > 0) {
				for (var j = 0; j < images.length; j++) {
					var arrayImage = images[j];
					var image = HTMLprojectImage.replace("%data%", arrayImage);
					$(".project-entry:last").append(image);
				}
			}
		}
	...
	renderHeader : function () {
		var bioObj = controller.getBioObj();

		// This will build the skills that are shown on the top of the page
		if (bioObj.skills.length > 0) {
			$("#header").append(HTMLskillsStart);
			for (var i = 0;i < bioObj.skills.length; i++) {
				var formattedSkill = HTMLskills.replace("%data%", bioObj.skills[i])
				$("#skills").append(formattedSkill);
			}
		}

		// This will render the contact infrmation on the page.
		var name = HTMLheaderName.replace("%data%", bioObj.name);
		var role = HTMLheaderRole.replace("%data%", bioObj.role);
		var pic = HTMLbioPic.replace("%data%", bioObj.picture);
		var message = HTMLwelcomeMsg.replace("%data", bioObj.welcome);
		var mobile = HTMLmobile.replace("%data%", bioObj.contact.mobile);
		var email = HTMLemail.replace("%data%", bioObj.contact.email);
		var github = HTMLgithub.replace("%data%", bioObj.contact.github);
		var location = HTMLlocation.replace("%data%", bioObj.contact.location);

		$("#header").prepend('<div style="float right;"><a ref="/" id="homereturn">Return to Home</a></div><hr id="topLine">');
		$("#header").prepend(role);
		$("#header").prepend(name)
		$("#header").append(pic);
		$("#header").append(message);
	...
	renderEducation : {
		Schools : function () {
			schoolsObj = controller.getSchoolsObj();
			...
			for (var i = 0; i < schoolsObj.length; i++) {
				...
				var name = HTMLschoolName.replace(/%link%|%data%/gi, function(matched) {
					if (matched === '%link%') {
						return schoolsObj[i].link;
					}
					else {
						return schoolsObj[i].name;
					}
				})

				var location = schoolsObj[i].location;
				var degree = HTMLschoolDegree.replace("%data%", schoolsObj[i].degree);
				var nameDegree = name + degree + location;
				var major = HTMLschoolMajor.replace("%data%", schoolsObj[i].major);
				var dates = HTMLschoolDates.replace("%data%", schoolsObj[i].dates);
				var seal = HTMLschoolSeal.replace("%data%", schoolsObj[i].seal);
		...
		Online : function () {
			var onlineObj = controller.getOnlineCoursesObj();

			$("#education").append(HTMLonlineClasses);
			$("#education").append('<hr class="dark-blue">');

			for (var i = 0; i < onlineObj.length; i++) {

				$("#education").append(HTMLbluestyling);
				$(".holder:last").append(HTMLschoolStart);

				var title = HTMLonlineTitle.replace("%data%", onlineObj[i].title);
				var school = HTMLonlineSchool.replace("%data%", onlineObj[i].school);
				var titleSchool = title + school;
				var dates = HTMLonlineDates.replace("%data%", onlineObj[i].dates);
				var urlhash = HTMLonlineURL.replace(/%data%|#/g, function (matched) {
					if (matched === "%data%") {
						return onlineObj[i].title + " ----Link";
					}
					else {
						return onlineObj[i].url;
					}
				});
...
controller = {
	...
	getSchoolsObj : function () {
		return model.schools;
	},

	getOnlineCoursesObj : function () {
